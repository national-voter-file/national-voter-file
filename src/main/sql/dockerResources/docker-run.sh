...
docker run --name nvf_postgis -p 5433:5432 -e POSTGRES_PASSWORD=$1 voterfile/postgis

# Open up the ports to allow remote access to db
iptables -A INPUT -p tcp -m tcp --sport 5433 -j ACCEPT
iptables -A OUTPUT -p tcp -m tcp --sport 5433 -j ACCEPT

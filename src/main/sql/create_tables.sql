...
CREATE TABLE VOTER_REPORT_SUMMARY (
    VOTER_REPORT_SUMMARY_ID BIGSERIAL PRIMARY KEY,
    REPORTER_KEY  INTEGER
      REFERENCES REPORTER_DIM(REPORTER_ID),
    JURISDICTION_KEY  INTEGER
      REFERENCES JURISDICTION_DIM(JURISDICTION_ID),
    ...
    REGISTERED_IN_LAST_90_DAYS INTEGER
  );


  DROP TABLE IF EXISTS CANVASS_ROUTE_DIM;

  CREATE TABLE CANVASS_ROUTE_DIM (
    CANVASS_ROUTE_ID BIGSERIAL PRIMARY KEY,
    ROUTE_TITLE TEXT,
    CREATION_DATE TIMESTAMP,
    CANVASSING_DATE DATE
  );


  DROP TABLE IF EXISTS CANVASS_STOP_FACT;

  CREATE TABLE CANVASS_STOP_FACT (
    CANVASS_STOP_ID BIGSERIAL PRIMARY KEY,
    VOTER_KEY
      BIGINT REFERENCES VOTER_DIM(VOTER_ID),
    HOUSEHOLD_KEY BIGINT
      REFERENCES HOUSEHOLD_DIM(HOUSEHOLD_ID),
    PARTY_KEY INTEGER
      REFERENCES PARTY_DIM(PARTY_ID),
    CANVASS_ROUTE_KEY BIGINT
      REFERENCES CANVASS_ROUTE_DIM(CANVASS_ROUTE_ID)
  )